...
const channelIcons: Record<BalanceChannel, { icon: JSX.Element; text: string }> = {
  [BalanceChannel.EMAIL]: { icon: <Mail width={16} />, text: "Email" },
  [BalanceChannel.SMS]: { icon: <Smartphone width={16} />, text: "SMS (coming soon)" },
};
  ...
  return (
    <>
      ...
      <p className="text-center text-white-muted text-lg mb-4">
        Fill out the receiver details. We'll send them your message and the withdrawal link with some onboarding
        resources.
      </p>

      <div className="w-full bg-white-muted/50 p-1 rounded-lg mb-2 flex">
        {Object.values(BalanceChannel).map((channel) => (
          <Button
            key={channel}
            type="button"
            ...
            variant="group"
            disabled={channel !== BalanceChannel.EMAIL}
          >
      ...
      {selectedchannel === BalanceChannel.EMAIL && (
        <div className="mb-2">
