...
const macaroonCreds = grpc.credentials.createFromMetadataGenerator(function (_args, callback) {
...
// Currently unused in favour of grpc client
...
export const promisifyGrpc = <T, U>(
  ...
  request: T
): Promise<U> => new Promise((resolve, reject) => grpcCall(request, (err, val) => (err ? reject(err) : resolve(val))));
